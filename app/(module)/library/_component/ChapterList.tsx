...
const ChapterCard = ({ chapter, onClick }: IChapterCard) => {
  ...
  return (
    <div
      className={"cursor-pointer p-3 rounded-lg hover:border-primary bg-muted border-b-2 border-muted"}
      onClick={() => onClick(id)}
    >
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Pagination className="flex justify-end pr-6 gap-6">
        ...
        <PaginationContent>
          <PaginationItem>
            {`${(pagination?.currentPage - 1) * pagination?.pageSize + 1} - ${Math.min(pagination?.currentPage * pagination?.pageSize, pagination?.totalElements)} of ${pagination?.totalElements}`}
          </PaginationItem>
        </PaginationContent>
        ...
        <PaginationContent>
          ...
          {Array.from({ length: pagination?.totalPages }).map((_, index) => (
            <PaginationItem>
              <Button variant={pagination?.currentPage === index + 1 ? "default" : "ghost"}>
                {index + 1}
              </Button>
            </PaginationItem>
          ))}
